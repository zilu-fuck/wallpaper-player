...
| 端 | 版本 | 产物 |
| --- | --- | --- |
| Windows 电脑端 | `1.4.0` | `release/Wallpaper-Player-Setup-1.4.0.exe`、`release/Wallpaper-Player-1.4.0.exe` |
...
本批更新说明见 [release-notes/v1.4.0.md](release-notes/v1.4.0.md)，开发记录见 [docs/dev-log-v1.3.0.md](docs/dev-log-v1.3.0.md)。
...
- 使用 mpv 播放常见桌面视频格式。
- 支持网络资源库，可保存视频直链、HLS/DASH、NAS/局域网链接和网页入口。
- 支持网页资源解析、剧集展开、内置网页兜底观看和网络资源编辑/删除。
- 新增下载中心，内置 aria2c，支持 HTTP/HTTPS 下载和 BT 磁链任务。
- 磁链下载支持元数据等待、文件选择、tracker、DHT、PEX、LSD、端口检测和完成后刷新目录。
- 支持检测并拉起本机迅雷，任务由迅雷接管时在应用内保留入口和保存目录。
- 支持深色/亮色主题。
- 支持手机端扫码绑定电脑。
- 手机端可在局域网内浏览电脑视频库。
- 手机端可浏览电脑端保存的网络资源，并通过电脑端代理播放直链、已解析网页视频和 HLS 分片。
- 手机端提供沉浸式上下滑动视频流。
...
1. 在电脑上安装或运行 `release/Wallpaper-Player-Setup-1.4.0.exe` / `release/Wallpaper-Player-1.4.0.exe`。
2. 打开电脑端，通过“添加资源”添加本地目录、网络资源或磁链任务。
3. 如需视频理解、AI 搜索或 Agent Bridge，在电脑端设置的“插件管理”中安装对应 `release/plugins/*.zip` 插件包，再启用插件。
4. 在电脑端设置中打开手机访问功能。
5. 在 Android 手机上安装 `mobile/dist/wallpaper-player-mobile-0.1.4-arm64-release.apk`。
6. 确保手机和电脑在同一局域网内。
...
```text
mobile/dist/wallpaper-player-mobile-0.1.4-arm64-release.apk
```
...
- [1.4.0 更新说明](release-notes/v1.4.0.md)
- [1.3.3 更新说明](release-notes/v1.3.3.md)
